...
- [ ] Extract remaining orchestration from `src/main.py` into service modules.
  Current progress: image provider selection, prompt translation, and backend generation moved to `src/core/image_service.py`; Telegram-side image job runtime moved to `src/adapters/telegram/image_runtime.py`; remaining launcher-owned work is now mostly chat/conversation orchestration plus bootstrap debt.
